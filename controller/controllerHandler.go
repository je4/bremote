...
func (css ControllerServiceServer) NewClient(ctx context.Context, param *pb.String) (*empty.Empty, error) {
	traceId, sourceInstance, targetInstance, err := common.RpcContextMetadata(ctx)
	if err != nil {
		css.log.Errorf("invalid metadata in call to %v: %v", "StartBrowser()", err)
		return nil, status.Errorf(codes.Unavailable, fmt.Sprintf("invalid metadata: %v", err))
	...
	css.log.Infof("[%v] %v -> %v/NewClient( %v )", traceId, sourceInstance, targetInstance, client)
	...
	go func() {
		...
		css.log.Infof("[%v] starting down browser of %v", traceId, client)
